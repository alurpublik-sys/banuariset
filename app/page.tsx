...
export default function Home(){
  ...
  useEffect(()=>{const timer=setInterval(()=>setSlide(v=>(v+1)%slides.length),6500);const obs=new IntersectionObserver(es=>{const v=es.filter(e=>e.isIntersecting).sort((a,b)=>b.intersectionRatio-a.intersectionRatio)[0];if(v?.target?.id)setActive(v.target.id)},{rootMargin:"-155px 0px -55% 0px",threshold:[.1,.4]});nav.map(([,id])=>document.getElementById(id)).filter(Boolean).forEach(el=>obs.observe(el!));return()=>{clearInterval(timer);obs.disconnect()}},[]);
  ...
  return <main id="top">
    ...
    <section id="publikasi" className="section"><div className="shell"><div className="title"><h2>Publikasi Terbaru</h2><i/></div><div className="pubgrid"><a href="#riset" className="feature"><img src="https://images.unsplash.com/photo-1523240795612-9a054b0db644?auto=format&fit=crop&w=1500&q=85" alt=""/><div><span>FEATURED RESEARCH</span><h2>Riset yang memberi konteks pada perubahan Sulawesi Tengah.</h2></div></a><div className="publist">{[["POLICY PAPER","Investasi pendidikan dan nilai publik daerah"],["POLICY BRIEF","Ketika data daerah harus berujung pada keputusan"],["INSIGHT","Membaca perubahan ekonomi dari perspektif wilayah"]].map((x,i)=><a href="#riset" className="pub" key={i}><span>{x[0]}</span><h3>{x[1]}</h3><b>→</b></a>)}</div></div></div></section>
    ...
    <section id="peneliti" className="section"><div className="shell"><div className="title"><h2>Peneliti</h2><i/></div>{cards([["RESEARCHER 01","Kebijakan Publik & Tata Kelola","Profil peneliti dan publikasinya."],["RESEARCHER 02","Ekonomi Regional & Industri","Profil peneliti dan publikasinya."],["RESEARCHER 03","Pendidikan & Pembangunan Sosial","Profil peneliti dan publikasinya."]],"#publikasi")}</div></section>

    <section id="data" className="section"><div className="shell"><div className="title"><h2>Sulawesi Tengah dalam Data</h2><i/></div><div className="stats">{[["13","Kabupaten/Kota"],["6","Fokus Riset"],["24+","Publikasi Target Awal"],["1","Regional Knowledge Hub"]].map((x,i)=><a href="#publikasi" className="stat" key={i}><strong>{x[0]}</strong><span>{x[1]}</span></a>)}</div></div></section>
